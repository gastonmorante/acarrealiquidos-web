...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 7. Interactive Booking Engine & Mercado Pago Integration
    const bookingForm = document.getElementById('booking-form');
    const routeSelect = document.getElementById('booking-route');
    const unitsSelect = document.getElementById('booking-units');
    ...
    // Price definitions (MXN)
    const routePrices = {
        'ver-cdmx': { es: "Veracruz ⇄ Cd. de México", en: "Veracruz ⇄ Mexico City", price: 22000 },
        'ver-qro': { es: "Veracruz ⇄ Querétaro / Bajío", en: "Veracruz ⇄ Querétaro / Bajío", price: 32000 },
        'ver-mty': { es: "Veracruz ⇄ Monterrey", en: "Veracruz ⇄ Monterrey", price: 45000 },
        'ver-custom': { es: "Cobertura Nacional", en: "National Coverage", price: 35000 }
    };
    ...
    function updateBookingSummary() {
        ...
        const units = parseInt(unitsSelect.value) || 1;
        const routeInfo = routePrices[routeKey];
        
        // Find selected payment type
        let paymentType = 'reserve';
        if (paymentRadios) {
        ...
        const subtotal = routeInfo.price * units;
        let total = 0;
        let termsText = '';

        if (paymentType === 'reserve') {
            total = 0;
            termsText = isEnglish ? 'BOOK ONLY (NO DEPOSIT)' : 'SÓLO RESERVA (SIN ANTICIPO)';
            if (submitBtn) submitBtn.classList.remove('hidden');
            if (mpContainer) mpContainer.classList.add('hidden');
        } else if (paymentType === 'partial') {
            total = subtotal * 0.15;
            termsText = isEnglish ? 'DEPOSIT (15%)' : 'ANTICIPO DE GARANTÍA (15%)';
            if (submitBtn) submitBtn.classList.add('hidden');
            if (mpContainer) mpContainer.classList.remove('hidden');
        } else if (paymentType === 'full') {
            total = subtotal;
            termsText = isEnglish ? 'FULL PAYMENT (100%)' : 'LIQUIDACIÓN TOTAL (100%)';
            if (submitBtn) submitBtn.classList.add('hidden');
            if (mpContainer) mpContainer.classList.remove('hidden');
        }
        ...
        // Update UI
        if (summaryRoute) summaryRoute.textContent = isEnglish ? routeInfo.en : routeInfo.es;
        if (summaryUnits) summaryUnits.textContent = isEnglish ? `${units} Unit(s)` : `${units} Unidad(es)`;
        if (summarySubtotal) summarySubtotal.textContent = formatCurrency(subtotal);
        if (summaryTerms) summaryTerms.textContent = termsText;
        if (summaryTotal) summaryTotal.textContent = paymentType === 'reserve' ? (isEnglish ? 'TBD / Quote' : 'Pendiente / Cotizar') : formatCurrency(total);
    }

    // Add listeners
    if (routeSelect) routeSelect.addEventListener('change', updateBookingSummary);
    if (unitsSelect) unitsSelect.addEventListener('change', updateBookingSummary);
    if (paymentRadios) {
    ...
    if (routeSelect) updateBookingSummary();

    // Mercado Pago Integration Placeholder
    const MP_PUBLIC_KEY = 'YOUR_MERCADO_PAGO_PUBLIC_KEY'; // Replace with real key

    // Simulated checkout execution
    function executeCheckout(paymentType) {
        const isEnglish = body.classList.contains('lang-en');
        ...
        if (!name || !email || !phone || !company || !date) {
            alert(isEnglish ? "Please fill in all contact details first." : "Por favor, complete todos los campos de contacto antes de continuar.");
            return;
        }
        ...
        if (MP_PUBLIC_KEY === 'YOUR_MERCADO_PAGO_PUBLIC_KEY') {
            ...
            const parent = bookingForm.parentElement;
            parent.innerHTML = `
                <div class="text-center py-12 px-6 flex flex-col items-center justify-center animate-fade-in" style="animation: fadeIn 0.6s ease-out;">
                    <div class="w-16 h-16 rounded-full bg-blue-50 border border-blue-500 flex items-center justify-center mb-6 text-blue-500 animate-bounce-in">
                        <span class="material-symbols-outlined text-4xl font-bold">hourglass_empty</span>
                    </div>
                    <h4 class="text-2xl font-bold text-deep-navy mb-4 font-headline-display">
                        ${isEnglish ? 'Mercado Pago Sandbox Mode' : 'Mercado Pago - Modo de Prueba'}
                    </h4>
                    <p class="text-sm text-slate-gray max-w-md mb-6 leading-relaxed font-body-md">
                        ${isEnglish ? 
                            'The Booking Engine is ready! Once you insert your Mercado Pago Public Key and Access Token credentials in `app.js`, this button will redirect directly to the secure payment checkout gateway.' : 
                            '¡El motor de reservas está listo! Cuando ingreses tu Public Key y Access Token de Mercado Pago en `app.js`, este botón redirigirá automáticamente a la pasarela de pago seguro en producción.'}
                    </p>
                    <div class="bg-gray-50 border border-gray-100 rounded-lg p-4 mb-8 text-left w-full text-xs font-body-md text-slate-gray space-y-2">
                        <p class="border-b border-gray-100 pb-1"><strong>${isEnglish ? 'Reserved Route:' : 'Ruta Reservada:'}</strong> ${summaryRoute.textContent}</p>
                        <p class="border-b border-gray-100 pb-1"><strong>${isEnglish ? 'Units:' : 'Unidades:'}</strong> ${summaryUnits.textContent}</p>
                        <p class="border-b border-gray-100 pb-1"><strong>${isEnglish ? 'Date:' : 'Fecha de Carga:'}</strong> ${date}</p>
                        <p class="border-b border-gray-100 pb-1"><strong>${isEnglish ? 'Customer:' : 'Contacto:'}</strong> ${name} (${company})</p>
                        <p><strong>${isEnglish ? 'Selected Scheme:' : 'Esquema:'}</strong> ${summaryTerms.textContent}</p>
                    </div>
                    <button type="button" onclick="window.location.reload()" class="bg-deep-navy hover:bg-safety-orange active:scale-95 text-white px-6 py-3.5 text-xs font-bold tracking-widest uppercase transition-all duration-300 rounded-lg shadow-md hover:shadow-safety-orange/20 cursor-pointer font-headline-display">
                        ${isEnglish ? 'Back to Form' : 'Regresar al Formulario'}
                    </button>
                </div>
            `;
        } else {
            // Real integration (Initialization code)
            try {
                const mp = new MercadoPago(MP_PUBLIC_KEY, { locale: isEnglish ? 'en-US' : 'es-MX' });
                alert(isEnglish ? "Redirecting to Mercado Pago secure checkout..." : "Redirigiendo a la pasarela segura de Mercado Pago...");
            } catch (err) {
    ...
    if (submitBtn) {
        submitBtn.addEventListener('click', () => {
            executeCheckout('reserve');
        });
    }

    if (payBtn) {
        payBtn.addEventListener('click', () => {
            executeCheckout('payment');
        });
    }

    // Listen to global language changes to also refresh summaries
